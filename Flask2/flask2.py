...
def get_and_post():
    # Process the data when SUBMIT is clicked, causing to POST.
    if request.method == 'POST':
        users = request.form['users']
        password = request.form['password']

        # Authenticate here and do action (redirection etc.)
        # In this case, I render the template again.
        return render_template('admin.html', users=users)
    else:
        # Return the site normally if they just GET.
